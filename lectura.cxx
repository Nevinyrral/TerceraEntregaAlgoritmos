...
typedef std :: vector< vector< bool > > Schedule;

bool student_comparator (Student& student_a, Student& student_b);
std :: vector<std :: string> scheduler (Student student, std :: map<Subject> subjects);
		    ...
		    for (int i = 0; i < subject_size; i++) {
		    	...
		    	for(int j = 0; j < classes_by_subject; j++){
					...
					if (previous < buffer.length()) {
		    			...
		    			day = std :: atoi (class_data[3].c_str() / 100);
...
std :: vector<std :: string> scheduler (Student student, std :: map<int, Subject>* subjects) {
	Schedule schedule (7, std :: vector<bool> day (24, false));
	std :: vector<std :: string> results;
	std :: vector<int> student_subjects = student.get_subjects();
	bool no_available_credits = false;
	int class_counter = 0;
	for (int i = 0; i < PASSES; i++) {
		while (student_subjects.size() < 0 and !no_available_credits) {
			// Choose random subject
			int random_subject = rand() % student_subjects.size();
			Subject* current_subject = (*subjects)[ student_subjects[random_subject] ];
			// TODO Verify classes for subjects, verify if they have seats and doesn't 
